...
setInfluent(
    0
)  # setting the influent for the initial time (t0) to be ready for the start of the simulation
...
for u in t[1:]:
    ...
    flowtemp_df = pd.DataFrame(flowtemp, index=[0])
    gasflow = pd.concat([gasflow, flowtemp_df], ignore_index=True, axis=0)
...
simulate_results.to_csv("dynamic_out_test.csv", index=False)
